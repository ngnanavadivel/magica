...
public class Automator {
	...
	public String automate(AutomatorRequest req) throws Exception {
		...
		List<String> fileNames = compiler
				.getFileNames(req.getGeneratedDir());

		compiler.compile(req.getCompiledDir(), fileNames.toArray(new String[0]));

		// 4. create JAR
		new JarMaker().createJar(req.getCompiledDir(),
				req.getJarFile());
		...
		if (req.srcFormat == FORMAT.XML) {
			...
			srcObject = unMarshaller.unmarshal(new StreamSource(new StringReader(req.sourceInstance)),
					Class.forName(req.getSourcePkg() + "." + req.srcRoot, true, jcl));
			...
			srcObject = mapper.readValue(new StringReader(req.sourceInstance),
					Class.forName(req.getSourcePkg() + "." + req.srcRoot, true, jcl));
		} else {
		...
		if (req.destFormat == FORMAT.JSON) {
			...
			marshaller.marshal(targetObject,
					writer);			
